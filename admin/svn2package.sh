#! /bin/sh
# 
# Generates a package from svn sources
#
# Usage: svn2package.sh [-b|--branch <svn-branch>] [packagename] 
# Stolen and adopted from Qwt library
# 
...
function checkout() {
    ...
    svn -q co https://trac.frm2.tum.de/qmesydaq/$1 /$3
    if [ $? -ne 0 ]
    then
        echo "Can't access SVN"
        exit $?
    fi
...
function clean {
    ...
    do
        expand -8 $EXPANDFILE > $EXPANDFILE.expand
...
#    if [ "$SUFFIX" != "" ]
#    then
#        sed -i -e "s/\$\$QWT_VERSION-svn/\$\$QWT_VERSION-$SUFFIX/" mesydaqconfig.pri 
#    else
#        sed -i -e "s/\$\$QWT_VERSION-svn/\$\$QWT_VERSION/"mesydaqconfig.pri
#    fi
...
while [ $# -gt 0 ] ; do
    case "$1" in
        ...
        *) 
            TARGETDIR=qwt-$1 ; VERSION=$1; shift;;
    esac
done
...
if [ "$SUFFIX" != "" ]
then
    TARGETDIR=$TARGETDIR-$SUFFIX
fi

TMPDIR=/tmp/$TARGETDIR-tmp

checkout $SVNDIR $BRANCH $TMPDIR
...
then
    ...
    if [ $GENERATE_PDF -ne 0 ]
    then
        mv $TMPDIR/doc/pdf/qwtdoc-$VERSION.pdf $TARGETDIR.pdf
        rmdir $TMPDIR/doc/pdf
    fi

    if [ $GENERATE_QCH -ne 0 ]
    then
        mv $TMPDIR/doc/html/qwtdoc.qch $TARGETDIR.qch
    fi
    echo done
fi
...
prepare4Unix $TARGETDIR
tar cfj $TARGETDIR.tar.bz2 $TARGETDIR

rm -rf $TARGETDIR
cp -a $TMPDIR $TARGETDIR
prepare4Win $TARGETDIR
zip -r $TARGETDIR.zip $TARGETDIR > /dev/null

rm -rf $TMPDIR $TARGETDIR

mv $TARGETDIR.tar.bz2 $TARGETDIR.zip $DIR/
echo done

 ...
 */
MCPD8::MCPD8(quint8 byId, QObject *parent, QString szMcpdId, quint16 wPort, QString szHostIp)
    : QObject(parent)
    , m_network(NULL)
    , m_txCmdBufNum(0)
    , m_id(byId)
    ...
    , m_dataPort(wPort)  // original 7000
    , m_master(true)
    , m_term(true)
    , m_extsync(false)
    , m_stream(false)
...
{
    stdInit();
    m_network = NetworkDevice::create(NULL, szHostIp, wPort);

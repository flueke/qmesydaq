...
CARESSControl::CARESSControl(QObject *parent)
	...
	, m_caressTaskNum(0)
{
	...
	m_ct = new CorbaThread(this);
	m_ct->start();
}

CARESSControl::~CARESSControl()
{
	if (m_ct->isRunning())
		m_ct->quit();
}
 ...
 */
bool CARESSControl::isActive(void)
{
 ...
 */
void CARESSControl::completeCar()
{
	m_caressTaskPending = false;
	m_asyncTaskPending = false;
	m_caressTaskNum = 0;
